...
export default function ArchiveDetail({params}:{params: Promise<{locale:string; id:string}>}) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const url = `${process.env.NEXT_PUBLIC_CMS_URL ?? ''}/api/dijital-miras-hikayesis/${id}`;
  ...
  return (
    <main className="p-8 max-w-3xl mx-auto">
      ...
      <p className="opacity-70 mt-2">{item.attributes?.category}</p>
    </main>
  );
}
