...
export default function ServiceDetail({params}:{params: Promise<{locale:string; id:string}>}) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const url = `${process.env.NEXT_PUBLIC_CMS_URL ?? ''}/api/hizmets/${id}`;
  ...
  return (
    <main className="p-8 max-w-3xl mx-auto">
      ...
      <p className="opacity-80 mt-2 whitespace-pre-line">{item.attributes?.description}</p>
    </main>
  );
}
